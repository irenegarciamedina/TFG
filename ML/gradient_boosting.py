...
# Se reutilizan las funciones de etiquetado implementadas en el SVM
from ML.SVM import etiquetar_caidas, etiquetar_todos_pacientes
...
def _lopo_cv(X: np.ndarray, y: np.ndarray, pac_ids: np.ndarray) -> dict:
    ...
    print(f"\n[GBT] Leave-One-Patient-Out CV  ({n_folds} folds)...")
    ...
    for fold_i, pac_test in enumerate(pacientes, 1):
        ...
        # Se aplica peso de clases para compensar desbalance (~16:1 ruido/caída)
        pos   = y_tr.sum()
        neg   = (y_tr == 0).sum()
        ...
        # GradientBoosting no tiene class_weight nativo -> se sobrepesa la clase minoritaria
        sample_weight = np.where(y_tr == 1, scale, 1.0)
        gbt.fit(X_tr, y_tr, sample_weight=sample_weight)

        y_pred = gbt.predict(X_te)
        y_prob = gbt.predict_proba(X_te)[:, 1]
...
def _evaluar_gbt(y_test, y_pred, y_prob) -> dict:
...
def ejecutar_gradient_boosting() -> dict:
    if not INPUT_FILES:
        print("[GBT] ⚠ No hay ficheros preprocesados disponibles.")
        return {}

    print(f"\n[GBT] Cargando {len(INPUT_FILES)} fichero(s)...")
    frames = []
    ...
    if not lopo:
        print("[GBT] ⚠ LOPO CV no produjo resultados válidos.")
        return {}

    metricas = _evaluar_gbt(lopo["y_test"], lopo["y_pred"], lopo["y_prob"])
    metricas.update({
        "y_test"         : lopo["y_test"],
        "y_pred"         : lopo["y_pred"],
        "y_prob"         : lopo["y_prob"],
    ...
    from ML.visualizacion import generar_dashboard_gbt, escribir_reporte_gbt
    print("[GBT] Generando dashboard y reporte...")
    generar_dashboard_gbt(metricas, df)
    escribir_reporte_gbt(metricas)
